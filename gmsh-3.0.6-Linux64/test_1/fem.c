...
// important - assigning boundary conditions
for(i=0;i<nnode;i++)
{
  if(coord[mnode*i+0]==0&&coord[mnode*i+1]!=0&&coord[mnode*i+1]!=1)
  {
  ...
  else if(coord[mnode*i+0]==1&&coord[mnode*i+1]!=0&&coord[mnode*i+1]!=1)
  {
    bcnode[k]=i+1;
    bcval[k]= sin(PI*coord[mnode*i+1]);
    k++;
  }
  else if(coord[mnode*i+1]==0)
  {
    bcnode[k]=i+1;
    bcval[k]=0;
    k++;
  }
  else if(coord[mnode*i+1]==1)
  {
    bcnode[k]=i+1;
    bcval[k]=0;
    k++;
...
// F[i] depicts the RHS of the governing equation - Poisson Eqn
k=0;i=0;iel=0;
for(i=0;i<sysdof;i++)
{F[i]=0;
  for(j=0;j<sysdof;j++)
