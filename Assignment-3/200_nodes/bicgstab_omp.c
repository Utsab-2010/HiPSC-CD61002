...
int main(int argc, char* argv[]) {
    ...
    char kmat_path[512];
    char fvec_path[512];
    snprintf(kmat_path, sizeof(kmat_path), "%s/Kmat.txt", dir_path);
    snprintf(fvec_path, sizeof(fvec_path), "%s/Fvec.txt", dir_path);
    ...
    if (read_matrix(kmat_path, &K, &n_matrix) != 0) {
        printf("Failed to read matrix file\n");
        return 1;
    }
    
    if (read_vector(fvec_path, &F, &n_vector) != 0) {
